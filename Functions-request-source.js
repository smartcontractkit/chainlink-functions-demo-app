...
const destAsset = args[1] //Token destination (buying)
const destDecimals = 6
const amount = args[2] * 1000000000000000000 // Amount of source token to trade

// Pull from the Paraswap DEX Aggregator router
const paraswapRequest = await Functions.makeHttpRequest({
  url: `https://apiv5.paraswap.io/prices?srcToken=${srcToken}&srcDecimals=${srcDecimals}&destToken=${destAsset}&destDecimals=${destDecimals}&amount=${amount}&network=1&side=SELL`,
})
...
const { exchange, data } = paraswapRequest.data.priceRoute.bestRoute[0].swaps[0].swapExchanges[0]
...
console.log(`Byte Size of return: ${Buffer.from(JSON.stringify(result)).length}`)
